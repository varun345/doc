...
> The transaction cannot succeed due to error: DutyRouter: INSUFFICIENT\_OUTPUT\_AMOUNT. This is probably an issue with one of the tokens you are swapping.
>
> the transaction cannot succeed due to error: execution reverted: dutyrouter: insufficient\_output\_amount.
...
However, there's also the chance that you're trying to trade a scam token which cannot be sold. In this case, DutySwap isn't able to block a token or return funds.
{% endtab %}
{% endtabs %}

### **INSUFFICIENT\_A\_AMOUNT or INSUFFICIENT\_B\_AMOUNT**

> Fail with error 'DutyRouter: INSUFFICIENT\_A\_AMOUNT'\
> or\
> Fail with error 'DutyRouter: INSUFFICIENT\_B\_AMOUNT'
...
There currently isn't a simple way to solve this issue from the DutySwap website: you'll need to interact with the contract directly. You can add liquidity directly via the Router contract, while setting amountAMin to a small amount, then withdrawing all liquidity.
...
> The transaction cannot succeed due to error: DutyRouter: EXPIRED. This is probably an issue with one of the tokens you are swapping.
...
> The transaction cannot succeed due to error: Duty: K. This is probably an issue with one of the tokens you are swapping.

Try modifying the amount on “To” field. Therefore putting "(estimated)" symbol on “From”. Then initiate the swap immediately.

![](<../.gitbook/assets/duty-k-solution (2).png>)

This usually happen when you are trying to swap a token with its own fee.

### Duty: TRANSFER\_FAILED

> The transaction cannot succeed due to error: execution reverted: Duty: TRANSFER\_FAILED.
...
{% tab title="Reason" %}
**This issue (while swapping) is caused by tokens which have hard-coded the V1 DutySwap router into their contract.**
...
You don't have enough SYRUP in your wallet to unstake from the DUTY-DUTY pool.

**Get at least as much SYRUP as the amount of DUTY that you’re trying to unstake.**

1. Buy SYRUP on the exchange. If you want to unstake 100 DUTY, you need at least 100 SYRUP.
...
This will unstake your staked tokens and lose any uncollected DUTY yield.

{% hint style="warning" %}
**This will lose any yield that you haven’t harvested yet.**
{% endhint %}

To stop this happening again, **don’t sell your SYRUP.** You still need it to unstake from the “Stake DUTY Earn DUTY” pool.

This error has happened because you have sold or transferred SYRUP tokens. SYRUP is minted in a 1:1 ratio to DUTY when you stake in the DUTY-DUTY Syrup Pool. SYRUP must be burned at a 1:1 ratio to DUTY when calling leaveStaking (unstaking your DUTY from the pool), so if you don't have enough, you can't unstake from the pool.
...
{% tabs %}
{% tab title="Solution" %}
Install the official browser extension to connect, or read our guide on [how to connect a wallet to DutySwap](https://docs.duty.exchange/get-started/connection-guide).
{% endtab %}
{% endtabs %}
...
### Oops! We couldn't find any Duty Collectibles in your wallet.
...
1. Go to “Collectible” page, then come back to profile page.\
   If you can’t find the link, go to [https://duty.exchange/collectibles](https://duty.exchange/collectibles) directly.
...
\
It may make a conflict between wallets. This is out of DutySwap's control and we can do nothing.
